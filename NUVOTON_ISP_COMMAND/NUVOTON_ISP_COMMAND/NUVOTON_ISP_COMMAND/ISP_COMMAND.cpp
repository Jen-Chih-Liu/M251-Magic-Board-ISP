...
void ISP_COMMAND::WRITE_APROM_IMG_RAWDATA_USB(unsigned char* temp, unsigned char w_length)
{
	clock_t start_time, end_time;
	float total_time = 0;
...
ISP_STATE ISP_COMMAND::File_Open_APROM(_TCHAR* temp)
{
	...
	file_checksum = 0;
	for (unsigned int i = 0; i < baprom_size-8; i++)
	{
...
void ISP_COMMAND::APROM_AND_CHECKSUM(void)
{
	...
	W_APROM_BUFFER[(APROM_SIZE)-8] = ((baprom_size-8) >> 0) & 0xff;
	W_APROM_BUFFER[(APROM_SIZE)-7] = ((baprom_size-8) >> 8) & 0xff;
	W_APROM_BUFFER[(APROM_SIZE)-6] = ((baprom_size-8) >> 16) & 0xff;
	W_APROM_BUFFER[(APROM_SIZE)-5] = ((baprom_size-8) >> 24) & 0xff;
